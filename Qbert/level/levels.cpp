    ...
    void load_test_level()
    {
        ...
        for (int i = 0; i < 7; ++i)
        {
            ...
            for (int j = 0; j < i + 1; ++j)
            {
                ...
                auto color_2 = sprite_manager::get_instance().load_sprite(qb_sp_level_2_yellow_cube_1, qb_re_t_sprite_general);
                go->add_component<sprite_component>();
                auto cube = go->add_component<cube_component>(std::vector{color_1, color_2}, true);
